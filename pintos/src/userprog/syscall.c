...
struct proc_file* list_search(struct list* archivos, int fd);
...
void matar_proc(int estatus){
    ...
    struct list_elem *e = list_begin(&thread_current()->padre->lista_proc_hijos);
    struct hijo *h;

    while(e != list_end(&thread_current()->padre->lista_proc_hijos))
    {
    	h = list_entry(e, struct hijo, elem);
...
struct proc_file* list_search(struct list* archivos, int fd)
{
  ...
  struct proc_file *f;
      ...
      for (e = list_begin(archivos); e != list_end(archivos); e = list_next(e))
		{
